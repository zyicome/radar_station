...
static uint8_t *img_buffer_host_armor = nullptr;
static uint8_t *img_buffer_device_armor = nullptr;
...
void cuda_preprocess_car(uint8_t *src, int src_width, int src_height, float *dst, int dst_width, int dst_height,
                     cudaStream_t stream) {
...
void cuda_preprocess_armor(uint8_t *src, int src_width, int src_height, float *dst, int dst_width, int dst_height,
                     cudaStream_t stream) {
    int img_size = src_width * src_height * 3;
    // copy data to pinned memory
    memcpy(img_buffer_host_armor, src, img_size);
    // copy data to device memory
    CUDA_CHECK(cudaMemcpyAsync(img_buffer_device_armor, img_buffer_host_armor, img_size, cudaMemcpyHostToDevice, stream));
    ...
    int blocks = ceil(jobs / (float) threads);
    warpaffine_kernel<<<blocks, threads, 0, stream>>>(
            img_buffer_device_armor, src_width * 3, src_width,
            src_height, dst, dst_width,
            dst_height, 128, d2s, jobs);
}
...
void cuda_batch_preprocess(std::vector<cv::Mat> &img_batch,
    ...
    for (size_t i = 0; i < img_batch.size(); i++) {
        cuda_preprocess_car(img_batch[i].ptr(), img_batch[i].cols, img_batch[i].rows, &dst[dst_size * i], dst_width,
                        dst_height, stream);
...
void cuda_preprocess_init_armor(int max_image_size) {
    // prepare input data in pinned memory
    CUDA_CHECK(cudaMallocHost((void **) &img_buffer_host_armor, max_image_size * 3));
    // prepare input data in device memory
    CUDA_CHECK(cudaMalloc((void **) &img_buffer_device_armor, max_image_size * 3));
}
...
void cuda_preprocess_destroy_armor() {
    CUDA_CHECK(cudaFree(img_buffer_device_armor));
    CUDA_CHECK(cudaFreeHost(img_buffer_host_armor));
}
